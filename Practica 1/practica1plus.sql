...
--Crear valoraciones
CREATE PROCEDURE gen_valoraciones 
...
@numUsu int,
@numComentarios int ,
@numComentarioIntroducido int,
@numComentariosPorUsuario int, 
@idComentario int,
@valoresInsertar int ,
@valoresInsertados int

set @valoresInsertados = 0;
...
--Primero obtenemos el numero de usuario, comentarios y valores a insertar en la base de datos
select @numUsu=count(*)*.05+1 from usuario;
select @numComentarios=count(*)*.25+1 from comentario;
select @valoresInsertar = @numUsu*@numComentarios/2;
select @numComentariosPorUsuario = @valoresInsertar/@numUsu;

	--Recorremos un bucle para cada usuario, inserte el numero de comentario establecido hasta llegar al @valoresInsertar
	--Obtenemos del comentario el nombre del usuario para indicarlo en @valoradoPor
	WHILE (@valoresInsertar > @valoresInsertados)	
	BEGIN
		--Seleccionamos un comentario aleatorio y un usuario aleatorio
		select @usuId = usuId from usuario order by NEWID();
		--Si algun usuario ya ha valorado se volvera a seleccionar otro usuario aleatorio.
		IF (select count(*) from valora where usuId = @usuId) = 0 	
		BEGIN
			set @numComentarioIntroducido = 0;
		--Mientras el numComentario por usuario sea menor de los introducidos se ejecutara este bucle
			WHILE (@numComentariosPorUsuario > @numComentarioIntroducido)
			BEGIN
		--Obtenemos el numero de estrellas de forma aleatoria entre 1 y 5 para cada iteracion
			set @estrellas =floor(rand()*5)+1;
			select @idComentario = numcom, @valoradoA = usuId from comentario order by NEWID();
				--Si ese comentario ya se ha valorado se volvera a seleccionar otro comentario aleatorio.
				IF (select count(*) from valora where usuId = @valoradoA AND numcom=@idComentario) = 0 	
				BEGIN
				--Añadimos la valoracion y subimos el contador
					insert into valora (usuId,valorado,numcom,estrellas) values (@usuId,@valoradoA,@idComentario,@estrellas)
					set @valoresInsertados = @valoresInsertados + 1;
					set @numComentarioIntroducido = @numComentarioIntroducido + 1;
				END
			END
		END
	END
